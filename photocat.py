#!/usr/bin/env python3
"""
PhotoCat: Catalog → Hash (file/content) → Propose → Export → Execute → Quarantine (SQLite-backed)

Highlights
- Tracks scan roots (audit with `roots`)
- File hash: full-file SHA-256
- Content hash (optional): pixel-only SHA-256 via ExifTool stream (ignores metadata)
- Propose/export/execute/quarantine as before
- Default execute action is COPY; use --use-hardlinks to hardlink
- Progress bars for hash, hash-content, propose, execute, quarantine
- NEW: Preserve “special” subfolders (e.g., bad, Originals, .picasaoriginals, any starting with '_', thumbnails, _vti_cnf)
       under the date folder in the destination library.
...
Typical usage (PowerShell/CMD):
  ...
  # (optional) python photocat.py --db index.db hash-content
...
# Folders to ignore for tag derivation AND to preserve beneath date folder in dest path
SPECIAL_DIRS_EXACT = {"bad", "originals", ".picasaoriginals", "thumbnails", "_vti_cnf"}
# Also treat ANY folder that begins with underscore as "special", e.g. "_private", "_temp"
def is_special_dir(name: str) -> bool:
    n = name.lower()
    return n in SPECIAL_DIRS_EXACT or n.startswith("_")

# Common non-semantic directory names to ignore for tags
...
CREATE TABLE IF NOT EXISTS op_log(
  id INTEGER PRIMARY KEY,
  file_id INTEGER,
  op   TEXT NOT NULL,                    -- scan|hash|hash-content|plan|copy|hardlink|tag|verify|skip|quarantine|error
...
def rel_parts(base: Path, path: Path) -> List[str]:
    ...
    try:
        ...
        # If not strictly under base, fall back to last few meaningful parts
...
def derive_tags(base: Path, path: Path, mode: str) -> List[str]:
    ...
    # Filter out generic/special dirs
...
def find_special_chain(base: Path, path: Path) -> List[str]:
    """
    Return the ordered list of special directory components (if any)
    between the scanned base root and the file's parent directory.
    """
    parts = rel_parts(base, path)
    specials: List[str] = []
    for p in parts:
...
def cmd_propose(con: sqlite3.Connection, library_root: str) -> None:
    ...
    for idx, (sha,) in enumerate(groups, 1):
        ...
        # NEW: preserve special dir chain beneath date folder
...
# ---------- CLI ----------
def main():
    ap = argparse.ArgumentParser(description="PhotoCat (SQLite): catalog/hash/hash-content/propose/export/execute/quarantine/roots")
